...
public class PagamentoApi {
	...
	public ResponseEntity<Pagamento>apiPagamento (HttpServletRequest req,@RequestBody Pagamento pagamento,@RequestBody Cartao cartao,@RequestBody Boleto boleto,@RequestParam("tipoPagamento") TipoPagamento tipoPagamento,@RequestParam("quantidade") int quantidade,
		  ...
		  if (!isValid) { 
				...
				HttpEntity<Pagamento> request = new HttpEntity<>(pagamento);
				ResponseEntity<Pagamento> responseEntity = restTemplate.postForEntity(fooResourceUrl ,request, Pagamento.class);
				...
				if(responseEntity.getStatusCode().is2xxSuccessful()) {
					Pagamento pag = responseEntity.getBody();			
	...
	public ResponseEntity<Pagamento> detalhePorId( @RequestParam("id") Long id,
			@RequestHeader(value = "Authorization", required = false) String Authorization) {
		...
		boolean isValid = jwtComponent.isTokenExpired(Authorization.substring(7));
		if (!isValid) { 
		Pagamento pagamento = pagamentoService.findByIdCompra(id);
			if(pagamento != null) {
				RestTemplate restTemplate = new RestTemplate(); 
